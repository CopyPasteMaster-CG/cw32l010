...
int ADC_Enable(void)
{
    ...
    //等待 ADC 模块启动完成
    while (!ADC_GetITStatus(ADC_IT_READY) && timeout--)
    {
        ;
    }
...
{
    assert_param(IS_ADC_ALTR_CH(ADC_AltrStruct->ADC_AltrChmux ));
    assert_param(IS_ADC_DMASOF_ENABLE(ADC_AltrStruct->ADC_DMASOFEn ));

    if (NULL == ADC_AltrStruct)
    {
        return;
    }

    ADC_Init(&ADC_AltrStruct->ADC_InitStruct);
		REGBITS_MODIFY(CW_ADC->CR0, ADC_CR0_MODE_Msk, ADC_ChannelByChannelMode);
    REGBITS_MODIFY(CW_ADC->CR1, ADC_CR1_DMASOFEN_Msk,
                   ADC_AltrStruct->ADC_DMASOFEn);
    REGBITS_MODIFY(CW_ADC->ALTR, ADC_ALTR_REPEAT_Msk | 0xFFFF,
                   ADC_AltrStruct->ADC_RepeatEn | (ADC_AltrStruct->ADC_AltrChmux) & 0xFFFF);
		
		if ((((ADC_AltrStruct->ADC_AltrChmux) & ADC_ALTR_CH15_Msk) == ADC_ALTR_CH15_Msk) || \
				(((ADC_AltrStruct->ADC_AltrChmux) & ADC_ALTR_CH14_Msk) == ADC_ALTR_CH14_Msk) || \
				(((ADC_AltrStruct->ADC_AltrChmux) & ADC_ALTR_CH13_Msk) == ADC_ALTR_CH13_Msk))
    {
        REGBITS_SET(CW_ADC->CR0, ADC_CR0_BUF_Msk);
    }

    //如果ADC通道选择了内置温度传感器输出电压，则需要使能内置温度传感器
    if (((ADC_AltrStruct->ADC_AltrChmux) & ADC_ALTR_CH14_Msk) == ADC_ALTR_CH14_Msk)
    {
        //使能内置温度传感器
        REGBITS_SET(CW_ADC->CR0, ADC_CR0_TSEN_Msk);
    }
 ...
 *        @arg ENABLE
 */
void ADC_AltrChannelCfg(uint16_t ADC_TRIG, FunctionalState NewState)
{
    uint16_t trigMask = 0;
    /* Check the parameters */
    assert_param(IS_FUNCTIONAL_STATE(NewState));
